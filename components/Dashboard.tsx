...
interface DashboardProps {
    ...
    activeGoals: any[]; // Using any[] for now to match the structure, ideally should be a defined type
}

export default function Dashboard({ onNavigate, activeGoals }: DashboardProps) {
                        ...
                        <section>
                            ...
                            <div className="grid sm:grid-cols-2 gap-4">
                                {activeGoals.length === 0 && (
                                    <div className="col-span-2 text-center py-10 border border-dashed border-zinc-800 rounded-2xl">
                                        <p className="text-zinc-500 mb-4">No active goals yet.</p>
                                        <button onClick={() => onNavigate('goal-intake')} className="text-[#5100fd] hover:underline">Create a new goal</button>
                                    </div>
                                )}
                                {activeGoals.map((goal, index) => (
                                    <div key={index} className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-2xl hover:border-[#5100fd]/50 transition-colors cursor-pointer group">
                                        ...
                                        <h3 className="text-xl font-medium text-white mb-1">{goal.goalTitle}</h3>
                                        <p className="text-sm text-zinc-400 mb-4">Next: {goal.nextTask || "Start Journey"}</p>
                                        <div className="w-full bg-zinc-900 rounded-full h-1">
                                            <div className="bg-[#5100fd] h-1 rounded-full" style={{ width: `${goal.progress || 0}%` }} />
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="space-y-0 relative">
                            ...
                            {timeline.map((item, idx) => (
                                <div key={idx} className={`relative flex gap-4 py-4 ${item.status === 'completed' ? 'opacity-50' : 'opacity-100'}`}>
                                    ...
                                    <div className={`relative z-10 w-3 h-3 rounded-full mt-1.5 border-2 ${item.status === 'current' ? 'bg-[#5100fd] border-[#5100fd] shadow-[0_0_10px_rgba(81,0,253,0.5)]' :
